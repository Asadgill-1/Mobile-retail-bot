...
### Stage 12h — gaps wave: date refs + delivery charges + offers + online-order IMEI (migration 023), 2026-07-21

Five owner-requested gaps, one migration (`023_offers_delivery_numbering.sql`, applied live). Everything below verified against real code + a live-DB end-to-end run — no hallucinated behavior.

- **Date-based order/invoice refs** — `ODR-DD-MM-NNN` / `INV-DD-MM-NNN` (day-month-daily sequence, per shop, resets at Dubai midnight). The bigint `order_number`/`invoice_number` keys are **kept** (humans still type `order_number` into bot commands, FKs still point at it) — the date code is a **display layer** computed from `created_at` + a new per-shop-per-day `day_seq`, allocated by RPC `next_day_seq(p_shop, kind, day)` off a `daily_counters` table. `utils/codes.py` gained `order_ref`/`invoice_ref` (+ self-check); the dashboard mirrors them in `lib/types.ts`. Pre-023 rows fall back to `#{order_number}` / `INV-000042`.
- **Home delivery charge** — `orders.delivery_fee` (keeper sets it at confirm: `/confirmorder <#> [fee]` on the bot, a field on the dashboard confirm; the inline ✅ button stays free-delivery). The customer confirmation shows a breakdown (item + delivery = total). `assign_delivery` COD = product + delivery. New per-shop setting `shops.rider_keeps_delivery`: when on, the rider keeps the delivery cash and the COD `collect` ledger row records **product-only** (`deliver_order` in `riders/service.py`); reports split "delivery collected" vs "kept by riders". Invoices add a delivery line; reports add delivery income.
- **Shop offers** — new `offers` table (one active per product, unique partial index). Six types: free_gift (stock-linked — the gift is a real product, its stock decrements, prints as a 0.00 invoice line via an `orders.applied_offer` snapshot), free_delivery, percent_off, amount_off, bogo, bulk. The AI **advertises** them: `search_products` attaches the active-offer label, `orchestrator._serialize` passes it as `"offer"`, and the system prompt tells the model to mention it. Auto-**applied at confirm** (both `orders/service.py::confirm_order` and the dashboard's `confirmOrder`, mirrored math — `_offer_discount` in Python, `lib/offers.ts::computeOffer` in TS). Dashboard CRUD is an Offers card on the product page (`actions/offers.ts`). *POS (walk-in) offers are cashier-manual by design — the cashier sets prices directly; auto-application is the online/AI path.*
- **Online-order IMEI** — the pick-&-pack sheet gained an `IMEI(s)` column (backend `utils/excel.py` + dashboard orders CSV, blank for the packer). At invoice time, `createInvoiceFromOrder` requires the IMEI(s) for Mobile/Tablet, flips those `product_units` to `sold` and sets the long-idle `order_id` FK (migration 022), and prints them on the invoice — reusing the P3 POS IMEI machinery. Online sales now keep the unit ledger accurate, not just walk-ins.
- **Live-verified E2E:** a draft with a free_gift offer, confirmed with a 25 AED delivery fee, produced `ODR-21-07-001`, `delivery_fee=25`, an `applied_offer` gift snapshot, and gift-stock `6→5` + main-stock `7→6` — all checked in the live DB, then cleaned up. Backend suite **503 green** (+1 offer-math test).

### Stage 12i — platform-owner console + runtime AI model switching (migrations 024–026), 2026-07-21

The third front end, in its own repo (`owner-dashboard-mobile`, Vercel). This one is for the
**platform owner** — the operator who sells the system to shops — not a shop owner. It replaces
the owner Telegram bot's admin surface with a web console and adds the one capability the bot
never had: changing which AI model answers customers, at runtime.

- **No bridge, again.** The plan in that repo called for ~12 internal FastAPI endpoints behind a
  Cloudflare Tunnel; dropped for the same reason as Stage 12g. Two tables carry everything:
  `platform_settings` (config + a health snapshot the 60s beat publishes) and `redis_ops` (an
  outbox the same beat drains through the real `security/service.py` functions). A snapshot that
  stops arriving IS the "backend offline" signal — no probe, no public address on the owner's PC.
- **Runtime AI switching (migration 024).** `LLMClient` now overlays `ai_provider`/`ai_base_url`/
  `ai_model`/`ai_api_key` from `platform_settings` over `.env` (env stays the floor; clearing a row
  reverts) and re-reads on a 60s TTL inside the async `chat()` path, rebuilding the HTTP client only
  when the endpoint or key changed. Every bot and worker picks up a model change within a minute,
  no restart. Keys live in the table by owner decision — RLS on with **no policies**, so only the
  service role can read them, and the console never ships one to the browser. The vision model
  stays env-pinned so counter-sheet OCR can't be broken by a chat-model switch.
- **`pipeline_events` (migration 025)** — one fire-and-forget insert in `pipeline.py` for every
  NON-ai outcome (suspended/blacklisted/quarantined/frozen/bypass/attack/too_long/rate_capped/
  locked/duplicate). Answers "why did this customer never get a reply?", which previously only
  existed for attacks. `ai` is deliberately not logged — the archive already has those.
- **Soft delete (migration 026)** — `delivery_persons.active`, shops gain `'archived'`
  (`ShopStatus.ARCHIVED`; `list_shops` now excludes them, so archived shops vanish from the bot
  builders too). Clients already had `'offboarded'`. Hard deletes stay impossible: orders,
  invoices and COD rows reference these tenants.
- **Live-verified end to end:** a shop-owner account was rejected by the console's allowlist while
  the platform owner got in; the beat's snapshot drove the health strip green then "offline" after
  3 minutes; a model set in the browser (`kimi-k2.5-turbo`) was reported by a backend that had
  booted on `kimi-k2.6`, and the Test round-trip returned the provider's real 404 for that made-up
  model (captured on the row, **health beat still ok**) then `{ok, kimi-k2.6, "ready", 3812ms}`
  once corrected; a full client→shop→keeper→dashboard-login chain was created and the login
  authenticated scoped to exactly that shop; archiving hid the shop from `list_shops`; a blocked
  message produced a `pipeline_events` row. All test artifacts removed afterwards.
  Backend suite: **515 green** (was 503; +12 across the console channel and the LLM overlay).

### Stage 12l — VAT charged on top, POS discounts, tax credit notes, live report drill-down (migrations 029–030), 2026-07-29

> Migration 028 (`shops.customer_channel`/`whatsapp_phone_id` — the per-shop WhatsApp cutover seam)
> landed in git history before this stage but has **no entry in this file** — a documentation gap
> from an earlier session, not this one. It is not re-created here; whoever built it should add its
> own Stage entry with real live-verification detail rather than have one invented after the fact.

Requested by the shop owner: retail prices needed VAT added at the till, not carved out of the
sticker price; discounts needed to be visible per sale, not just per online order; invoices needed
to say which channel sold them; and every Reports figure needed to open into the rows behind it.

- **VAT was wrong, not just missing.** `products.selling_price`/`orders.selling_price`/
  `counter_sales.sold_price` are the price a shop types into inventory — **before VAT** — but every
  money path (POS checkout, the AI's quoted price, a haggled price, COD, invoice VAT) treated it as
  VAT-**inclusive** and divided by `1.05` instead of multiplying. A shop pricing a phone at 1,350 was
  quietly absorbing the 5% out of its own margin on every sale, and every invoice under-declared
  output VAT (Stage 12f's own live-verify note above — `161.86` on a `3,399` counter sale — is the
  **bug**, not a correct reference figure).
- **One rule, one place each side.** `src/app/utils/vat.py` (`with_vat`/`without_vat`) is the only
  conversion point in this repo; `lib/money.ts` (`vatOnNet`/`withVat`) is its byte-for-byte TS twin.
  VAT is added at exactly two boundaries — anything a **customer** is quoted, offered, or charged,
  and the cash a **rider** collects (`cod_amount`) — and comes back off exactly once: a price the
  customer **names** while haggling arrives VAT-inclusive and must be net before it can meet a floor,
  a `min_price`, or a list price. Everything between those boundaries (floors, `min_price`,
  `discount_amount`, every revenue figure in every report) stays net, so margins don't inflate 5%
  and the Python/TypeScript reconciliation (Stage 12f) still holds to the fil.
  - `ai/orchestrator.py::_serialize` now quotes `with_vat(selling_price)`; a customer's stated budget
    is converted `without_vat` before it filters the (net) catalogue.
  - `orders/service.py::request_price`/`approve_price`/`deny_price`/`confirm_order` now say the
    VAT-inclusive price to the customer while every internal comparison (`bargain_floor`,
    `discount_amount`) stays net.
  - `orders/service.py::assign_delivery` — COD is now `with_vat(item_net + fee)`, proved live to
    equal the dashboard's invoice total to the fil across 5 price shapes (including `0.01` and
    `9999.99/0.99/9.99`) via `scripts/pos_integration_check.py`.
  - `llm/prompts.py` — the system prompt states prices already include VAT; the model must not add
    tax on top or mention it unasked.
- **POS discounts (migration 030 — `counter_sales.discount_amount`, `invoices.discount`).** A
  cart-level giveaway is split across sale rows in whole fils (`allocateDiscount`, floor + hand the
  rounding remainder to the biggest line) so "who discounted what" is answerable per product; a void
  carries the **negative** discount so a reversed giveaway stops counting as money given away.
  `orders/service.py::merge_counter` folds it in exactly like an online order's `discount_amount` —
  revenue stays GROSS, the discount reports beside it, `net = revenue − discounts` holds on both
  channels. A void is also no longer double-counted as a sale (`merge_counter`'s `orders` count now
  excludes reversing rows, matching what `profit-math.ts` already did).
- **Tax credit notes (migration 029 — `invoices.credit_of`/`reason`).** A voided counter sale or a
  cancelled-after-invoice order used to leave its tax invoice, and its output VAT, standing — FTA
  rule: a reversed supply is reversed with a credit note, never by erasing the document.
  `lib/credit-note.ts::issueCreditNote` mirrors the original with negative amounts, continues the
  **same** per-shop sequence, is idempotent (a unique index backs it), and is wired from both
  `voidSale` and `cancelOrder` — the second call site was found unprompted while fixing the first.
- **Every Reports card now opens its own rows** (`app/(app)/reports/sales/page.tsx`, dashboard
  repo) — Orders/Total/Online/Counter/Discounts/Cost/Profit/Margin all link to the actual sale rows
  a headline was summed from, both channels, with a link to each invoice; the page's own totals
  restate the card that sent you (a void is a row here but not a sale, matching the card). Not
  owner-gated — a keeper checking their own day needs this as much as the owner does. The console
  (`owner-dashboard-mobile`) gained the same Online/Counter split on its platform-wide Analytics and
  per-shop table.
- **Channel badge** ("Counter sale"/"Online sale") added to the invoice list and the printed tax
  invoice/credit note; a credit note keeps the badge of what it reverses. Every document now prints
  Gross → Discount → Subtotal → VAT → Total in that fixed order, `0.00` discount included, so a
  reader sees that nothing was discounted rather than inferring it from an absent line.
- **The 5 pre-existing live invoices were retroactively restated**, not left wrong: a one-off script
  (not shipped — the fix is in the code, this was a one-time data correction) recomputed each
  invoice's subtotal/discount/VAT/total from its own source rows (`counter_sales`/`orders`, the only
  ground truth — the invoice's internal arithmetic is consistent either way and can't tell you which
  assumption produced it) and a credit note's from its original. `counter_sales`/`orders` themselves
  needed **no** correction — they already held the ex-VAT price; only the invoices had carved VAT out
  instead of adding it on. Idempotent — a second run finds nothing to restate. `orders.cod_amount`/
  `cash_received` on already-delivered orders were left untouched by design (they record cash a rider
  actually collected; restating them would falsify what happened).
- **Live-verified**, not just unit-tested: a real POS sale + discount + void on Shop 01 through the
  actual UI, `scripts/pos_integration_check.py` extended with VAT-on-top, discount-allocation, and
  COD-equals-invoice-total checks (all against a disposable shop, full teardown), both dashboards
  rebuilt clean. Backend suite: **608 green.** (Not stated as a delta from 12k's 561 — migration 028
  landed, untracked, somewhere in between; whoever documents it should reconcile the true count.)

## 🔵 In progress

- _(nothing in this backend repo — awaiting Stage 13. The web dashboard is a separate repo: P1–P4 + the Stage 12h gaps wave all done and live-verified.)_
...
| 2026-07-21 | 12h | **Gaps wave: date-based ODR/INV refs + home-delivery charges + shop offers + online-order IMEI (migration 023, applied live).** `daily_counters` + `next_day_seq` RPC drive `ODR-DD-MM-NNN`/`INV-DD-MM-NNN` display refs (bigint keys kept — codes are a display layer; `utils/codes.py::order_ref`/`invoice_ref`). `orders.delivery_fee` set at confirm with a customer breakdown; `shops.rider_keeps_delivery` splits COD reconcile (rider-kept delivery → product-only `collect` row) and reports (delivery collected vs kept). `offers` table (one active/product, 6 types); stock-linked free gift decrements the gift + prints a 0.00 invoice line via `orders.applied_offer`; the AI advertises offers (`search_products` + `_serialize` + prompt); auto-applied at confirm (Python `_offer_discount` mirrors TS `computeOffer`). Online-order IMEI: pick-&-pack sheet gains an IMEI column (`utils/excel.py`), invoice-from-order requires + records it (`product_units.order_id` finally used). POS offers stay cashier-manual by design. Live E2E verified (ODR ref + delivery_fee + gift snapshot + gift/main stock decrement). 1 new test (503 total) | Stage 12h |
| 2026-07-21 | 12i | **Platform-owner console (separate repo `owner-dashboard-mobile`) + runtime AI model switching (migrations 024–026, applied live).** Third front end, for the SaaS operator (NOT a shop owner): onboarding/offboarding, analytics, health, security ops, escalations, message archive, audit. **No bridge/tunnel** — `platform_settings` carries config + a health snapshot the 60s beat publishes (stale snapshot = "backend offline"), `redis_ops` is an outbox the same beat drains through the real security functions. `LLMClient` overlays provider/base_url/model/api_key from `platform_settings` over env on a 60s TTL, so a model switch (Moonshot/OpenAI/OpenRouter/any OpenAI-compatible) reaches every bot within a minute with no restart; keys are service-role-only and never sent to the browser; the vision model stays env-pinned. `pipeline_events` logs every non-AI pipeline outcome ("no message silently lost"). Soft delete: `shops 'archived'` (hidden from `list_shops`, so bots stop being built) + `delivery_persons.active`. Console access is an `OWNER_EMAILS` allowlist checked before auth — a shop owner with valid credentials is refused. Live-verified: AI switch observed by a running backend, provider round-trip (real 404 then ok/3812ms), full onboarding chain incl. a working provisioned keeper login, archive hiding, pipeline event capture. 12 new tests (515 total) | Stage 12i |
...
| 2026-07-29 | 12l | **VAT charged on top (was wrongly carved out), POS discounts, tax credit notes, live report drill-down (migrations 029–030, applied live).** `products.selling_price` is stored BEFORE VAT but every money path divided by 1.05 instead of multiplying — the shop was absorbing 5% out of its own margin and every invoice under-declared output VAT. `utils/vat.py` (`with_vat`/`without_vat`) + TS twin `lib/money.ts` now convert at exactly two boundaries (customer-quoted/charged, rider-collected COD) and back off once (a haggled price named by the customer). `counter_sales.discount_amount`/`invoices.discount` (030) let a POS giveaway be split per line (`allocateDiscount`) and reported like an online order's; `invoices.credit_of`/`reason` (029) let a void/cancel reverse its output VAT with a credit note instead of leaving it standing. Reports pages (both dashboard repos) gained a live drill-down — every card opens the actual rows it was summed from — and invoices/credit notes gained a channel badge. 5 pre-existing live invoices retroactively restated from their own source rows (one-off script, not shipped); `counter_sales`/`orders` needed no correction. Live-verified: real POS sale+discount+void on Shop 01, `pos_integration_check.py` extended (VAT-on-top, discount split, COD==invoice-total across 5 price shapes). 608 tests green | Stage 12l |
| 2026-07-25 | 12k | **Humanized the customer chat (migration 027, applied live).** Driven by real transcripts in `messages`, not by taste: production replies opened "I'd be happy to help you find a mobile!", asked for name/address/quantity as a numbered form, sent `**bold**` that the sender (no `parse_mode`) delivered as literal asterisks, told customers to visit the store and that "the shop will confirm" — both explicitly forbidden by the prompt — and landed in ~200ms as one block. `llm/prompts.py` keeps every anti-hallucination rule verbatim and gains what was missing: a VOICE block (no markdown, no em-dash, one question at a time, mirror the customer's own romanised Hindi/Urdu/Tagalog/Arabic), a consultative method (diagnose by use, translate spec→benefit, 2 options + a recommendation), a 5-step bargaining ladder, and a hard no-invention block (specs only from `specs`; no invented policy/warranty/instalments → escalate; scarcity only from real `in_stock`). Persona per shop (`assistant_name`/`assistant_gender`/`assistant_style`) — **gender is grammatical, not cosmetic**: Hindi/Urdu/Arabic conjugate by speaker gender and a "Sara" writing `sakta hoon` is an instant tell; `assistant_style` is shop-written text rendered inside the system prompt, so it is capped, newline-stripped and fenced. **Bargaining authority**: `shops.haggle_ask_every_time` (default true = unchanged behaviour) + `ai_max_discount_pct` + optional `products.min_price` → `bargain_floor` = product floor if set else % off list, clamped up to `cost_price` and rounded UP. At/above floor auto-approves, below counters AT the floor, and anything below a price already granted goes to a human. Manipulation-proof by construction: the charged price comes from a `price_requests` row only this server-side logic writes, so "your colleague promised me 1000" changes nothing. `offers.reveal='on_haggle'` holds a freebie back as a bargaining chip. **Pacing** (`telegram_bot/pacing.py`): one worker per conversation coalesces rapid fragments into a single answer, types with an indicator, and splits into ≤3 bubbles; a message arriving mid-LLM is answered next, never dropped; `concurrent_updates=True` on customer bots (required — PTB is otherwise strictly sequential per bot, so one typing pause would stall a whole shop) plus an `ai_max_concurrency` semaphore for the 300-message peak. `_serialize` stops leaking `high_margin` to the model. **Three defects found by the live run and fixed:** the model quoted "2600 tak kar sakti hoon" with no `request_price` call (prompt's ladder and tool contract were in tension — every step is now explicitly a tool call); the held-back freebie was spent in the opening reply (moved out of the product listing into the `request_price` result, so it cannot exist before bargaining starts); `maybeSingle()` on offers would throw once a product carried both an advertised and a held-back offer. Live-verified end to end: correct feminine forms throughout, Roman-Urdu mirroring, spec→benefit explanation, one recommendation with a runner-up, no invented price, policy questions and "are you a bot?" both escalating. `/addproduct` 12→13 steps (skippable floor). 42 new tests (561 total) | Stage 12k |
| 2026-07-31 | 12m | **POS completeness wave — security, numbering, stock ledger (migrations 032–034, all applied live).** Driven by a full audit of the shop + shop-owner accounts against UAE FTA rules and standard retail-POS practice. **(032) Live breach closed:** migration 006 sealed the data API, but 010/022/023/025 each pasted the old `using (true)` scaffold back onto new tables and Supabase's default privileges granted them — the **public anon key from the dashboard's browser bundle could read 12 tables** (invoices, messages, counter_sales, cod_ledger, dashboard_users, product_units, invoice_counters, daily_counters, offers, platform_settings, pipeline_events, redis_ops) and `DELETE /invoices` returned 204. Re-ran 006's sweep loop, dropped every policy, and revoked **default privileges** so the rot cannot recur; all 25 tables now 401. The three `public` functions keep an `anon=X` EXECUTE grant but are `security invoker`, so it buys nothing (probed live — all three refused). `docs/05-CONVENTIONS.md` had claimed "RLS still enforces `shop_id`" — false, corrected, and rule 6 now requires the two lines on every new table. **(033) Invoice numbers cannot be burned:** `next_invoice_number`/`next_day_seq` each committed in their own PostgREST round-trip before the app INSERTed separately, so any failure in between spent a number for good — Shop 01 is permanently missing `invoice_number` 3 and 4 and `day_seq` 1 of Dubai day 2026-07-29 (`INV-29-07-001` never printed). A `BEFORE INSERT` trigger now assigns both inside the insert's own transaction, only when the column arrives NULL (so it was safe to apply before the dashboard deploy). Historic gaps deliberately **not** back-filled — an allocated number with no supply is an accounting declaration, not something a migration invents. **(034) Every stock movement is journalled:** new append-only `stock_moves` + `move_stock` writing balance and journal in one transaction, keeping 003's `quantity >= n` guarantee as `quantity + delta >= 0`. `decrement_stock` redefined in place as a shim so all 12 call sites kept working with zero app changes, then every one migrated to real reasons (`sale`/`cancel`/`void`/`rollback`/`gift`/`intake`/`adjust`) with `ref_table`/`ref_id` back to the document — Python through its single wrapper, the dashboard through a new `lib/stock.ts`. `decrement_stock` now has **zero callers**, which makes it a detector: a `legacy` row means a stock writer nobody knows about. An AFTER INSERT trigger on `products` catches the two paths that set quantity outside the RPC. Standing invariant: `products.quantity` = `Σ stock_moves.delta`, returns 0. `product_id` is `on delete restrict` (cascade would erase the history the table exists for), so `deleteProduct` gained an honest guard. **Dashboard side (separate repo):** date-range periods (`YYYY-MM-DD..YYYY-MM-DD`, rejecting rolled-over dates — `Date.UTC(2026,1,31)` is silently 3 March), CSV exports for the invoice register / stock list with valuations / reports each running the same query as its page, the order-to-invoice queue un-capped from 10 and paged, and `ROLE-MATRIX.md` + `lib/role-parity.test.ts` pinning which keeper-vs-owner differences are deliberate. Live-verified under both roles: `/logs/export` is the one surface answering 404 to a keeper and CSV to an owner; a keeper **can** edit the TRN that prints on every tax invoice. 636 tests green, `pos_integration_check.py` 44/44 | Stage 12m |
| 2026-08-01 | 12n | **Manager PIN + approvals log (migration 035, applied live).** Phase 1d had measured it under both logins: role is consulted in exactly five files and **every destructive action is identical for a keeper and the shop owner** — a keeper could hard-delete a product, edit `cost_price`, void any sale, cancel any order, and edit the shop's TRN, with no ceiling on any of it. The answer is not owner-only locks (a keeper who cannot discount cannot serve a haggling customer): `manager_pins` (one row per **shop**, not per login) + append-only `override_approvals` put a PIN in front of the **loosening direction only** — deeper discount, lower price, bigger void, stock correction, cost edit, TRN change — while list price, a price rise, a restock and a small cancel stay one tap. **The 1-AED-phone hole is closed:** `checkoutSale` never loaded `selling_price`/`cost_price`/`min_price`, so a 4,000 AED phone could ring at 1 AED with `discount_amount = 0` and never appear in any discount report; the unit price is now gated against all three, read from the products row inside the same action. Hashing is Node `crypto.scrypt` + an env pepper, **not pgcrypto** — `crypt('123456', …)` travels in the SQL body and lands in the query log; only the digest reaches the database and the pepper never does. Lockout lives on the row (no Redis in the dashboards, and an in-memory counter resets on every cold start): 5 strikes → `2^(n-5)` minutes capped at 60, checked *before* the 64ms hash so a wrong-PIN storm cannot pin the event loop. **No override token** — the PIN is verified inside the same server action that performs the write, after `getScope()`/`assertShop`, so there is no window and nothing to replay. **Fails open, loudly:** a shop with no PIN row is blocked from nothing, but every over-threshold action logs `outcome='unset'`, so the owner's first look is a priced list of what has been happening unsupervised. Setting the PIN is **owner-only** — the one role gate this phase added, because a ceiling the gated person can move is not a ceiling. Surfaced as a 4th view on the existing owner-gated `/logs` plus its CSV: zero new routes, zero new auth surface. **Defect found by the live run:** React 19 resets an uncontrolled form once its action returns, so carrying the PIN as a form *field* re-sent the ORIGINAL values on retry — on the TRN a silent no-op (nothing differs, so nothing is gated, so nothing changes); every form caller now replays the submitted FormData via `usePinRetry`. Live-verified as `shop1@shop.ae` with a temporary PIN: a 3,399 AED iPhone at 1 AED refused with *"BELOW COST AED 2600.00"* and **nothing written** (no sale, no invoice, no stock move), wrong PIN counted down, the 5th locked the row and withdrew the prompt, a correct PIN approved a ±1 stock adjust (ledger still balances) and a TRN change (restored the same way). 636 tests green, dashboard 41 (`node --test`), `pos_integration_check.py` 49/49 including a cross-language scrypt check — if Node's defaults ever move, every till locks at once, so Python recomputes the same digest. **Owner half verified the same day:** the PIN card renders on both shops and is absent for a keeper, the owner set a real PIN on each **through the UI** (the supabase-js upsert was the last untested path), the Approvals view and its CSV both serve all six rows, and a +1 stock adjust raised the same prompt for the **owner** — the PIN is the shop's, not a role gate, which is what keeps `outcome='approved'` meaningful rather than a synonym for "a keeper did it". One display bug fixed there: the view printed `limit AED 5` against a `stock_adjust` whose threshold is 5 **units**, sitting directly under a real `AED 2,600` price ceiling; `limitText()` now follows `kind` (units / percent / AED) while the CSV keeps raw numbers, which is what a spreadsheet wants. **Deployment step still open: `OVERRIDE_PEPPER` must be set in Vercel** — without it every gated action refuses with "Manager approval is unavailable", deliberately failing closed rather than hashing with no pepper. | Stage 12n |
| 2026-08-01 | 12o | **Purchases + the shop's own VAT facts (migration 036, applied live).** Everything through 035 was the **output** side of VAT — what the shop sold and the 5% it collected. A shop that files only that pays over its entire output VAT and recovers nothing, which on a phone retailer's margins is the margin. New `suppliers` + `purchase_invoices` (**header only** — a purchase's lines exist to move stock, and that is 037; everything a VAT201 needs is on the header, and a booking that takes fifteen seconds per bill actually gets done, while one that demands every line does not). **The one control that matters is `unique (shop_id, supplier_id, upper(btrim(supplier_invoice_no)))`:** claiming a supplier invoice twice is the commonest input-VAT error and the easiest for the FTA to find, since they hold the supplier's copy — and it is rarely fraud, it is the same bill arriving by WhatsApp and in the box and being booked by two people. Case- and whitespace-insensitive on purpose, so `INV-0091`, `inv-0091 ` and `  inv-0091` are one document; per **supplier**, because two suppliers really do both use "INV-1". **Deliberately no CHECK that `vat_amount ≈ subtotal × 5%`** — real bills round per line then total and land a fil or two out, partly-exempt bills are legitimately far off, and a reverse-charge import carries no supplier VAT at all; a constraint would reject true documents and force whoever is booking to retype the paper until the form accepted it, which is how wrong numbers get filed. `vatVarianceWarning` warns past **±0.02** and books it anyway, wording it *"Booked as entered — check the bill."* `supplier_id` is `on delete restrict` so a supplier's tax records never vanish with the supplier — probed live, a bare supplier delete is refused while deleting the whole shop still cascades both tables in one statement. Also `shops.emirate` (standard-rated supplies report per emirate, VAT201 boxes 1a–1g; left NULL and badged missing rather than defaulted, because a default of 'Dubai' would silently file another emirate's numbers under Dubai), `shops.vat_period` (monthly/quarterly) and `shops.vat_quarter_anchor` — the FTA **staggers** quarters, so two thirds of quarterly filers are not on calendar quarters and a report that assumes Mar/Jun/Sep/Dec shows most of them the wrong period. Changing the emirate is **manager-PIN gated** (new kind `emirate`, 035's machinery), exactly like the TRN: it re-files a period's sales under a different heading, and it is a value that legitimately never changes. `invoices.vat_treatment` added as the output-side twin, default `standard`, sharing one word list (`standard · zero_rated · exempt · reverse_charge · out_of_scope`) with the purchase side so phase 5 reads one vocabulary for both halves of the return. Dashboard: a `/purchases` register with net / input VAT / **claimable** cards, the booking form (VAT auto-fills at 5% until someone types their own, then it stops overwriting — the partly-exempt bill is exactly the one where 5% is wrong), an optional photo of the bill straight to the existing private `shop-media` bucket via the product-media signed-upload flow, and a CSV running the same query as the page. **Live-verified end to end as `owner1@owner.ae` on Shop 01:** booking a bill, then the same bill typed `"  inv-7001 "` refused with *"already booked — claiming it twice is what the FTA looks for"*; a 1,000 net with 100 VAT **booked** with *"VAT 100.00 is 50.00 off 5% of 1000.00"*; cards, register and CSV agreeing to the fil; changing only the filing period saved with no prompt while changing the emirate raised *"Manager PIN required — Emirate (none) → Dubai"* and wrote nothing — and, as 035 specifies, no `override_approvals` row, because a prompt appearing is not a decision. Every probe row was deleted afterwards: a fabricated supplier bill is a tax record, unlike an approvals row. 636 tests green, dashboard 48 (`node --test`), `pos_integration_check.py` 58/58. | Stage 12o |
